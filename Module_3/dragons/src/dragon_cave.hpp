...
typedef std::list<Dragon*> DragonListType;

class DragonCave{
    public:
        DragonCave()=default;
        DragonCave(const DragonCave&)=delete;
        DragonCave& operator=(const DragonCave& dra)=delete;
        ~DragonCave();
        const DragonListType& GetDragons()const;
        void Accommodate(Dragon* dragon);
        void Evict(const std::string& draname);

    private : 
        DragonListType dragonlist;

};

std::ostream& operator<<(std::ostream& os, const DragonCave& drac);
