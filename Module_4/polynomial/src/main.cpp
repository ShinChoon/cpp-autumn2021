...
int main()
{
    ...
    {
        Poly p1, p2;
        std::istringstream iss("0x3+54x2-65x1-61x0 -1x2+3x4+2x-2+5x2");
        if (iss >> p1 >> p2)
        {
            std::cout << "p1: " << p1 << ", should be\n    20x4+15x3-10x2+5x1+x0" << std::endl;
            std::cout << "p2: " << p2 << ", should be\n    3x4+4x2+2x-2 (note the negative exp)" << std::endl;
        }
        else
            std::cout << "Reading from stream failed (shouldn't have failed)" << std::endl;
        const Poly &p1ref = p1;
        const Poly &p2ref = p2;
        std::cout << ((p2ref < p1ref) ? "true" : "false") << std::endl;
