...
class TranscriptRegistry{
    public:
        TranscriptRegistry(){}
        void Add(const Transcript& TR);
        void RemoveById(const std::string &SI);
        std::vector<Transcript>::const_iterator FindTranscript(const std::string &SI)const;
        std::list<std::pair<std::string, size_t>> FindCourseResults(const std::string &name)const;
        const std::vector<Transcript> GetTranscripts()const;

    private:
        std::vector<Transcript> TRlist;
};
